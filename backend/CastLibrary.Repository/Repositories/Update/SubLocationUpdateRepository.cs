...
namespace CastLibrary.Repository.Repositories.Update
{
    ...
    ICorrelationContext correlation) : ISublocationUpdateRepository
    {
        public async Task<SublocationDomain> UpdateAsync(SublocationDomain sublocation)
        {
            ...
            var @params = new { sublocation.Id, sublocation.LocationId, sublocation.Name, sublocation.Description, sublocation.DmNotes, sublocation.Keywords };
            ...
            var rows = await conn.ExecuteAsync(
                "UPDATE sublocations SET location_id=@LocationId, name=@Name, description=@Description, dm_notes=@DmNotes, keywords=@Keywords::text[] WHERE id=@Id",
                @params, tx);
