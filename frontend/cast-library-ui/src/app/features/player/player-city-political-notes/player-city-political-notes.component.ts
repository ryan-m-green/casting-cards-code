...
import {
  CityPoliticalNotes, CityFaction, CityFactionRelationship, CityNpcRole,
...
const EMPTY_NOTES = (): CityPoliticalNotes => ({
  id: '', campaignId: '', cityInstanceId: '',
...
@Component({
  selector: 'app-player-city-political-notes',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './player-city-political-notes.component.html',
  styleUrl: './player-city-political-notes.component.scss',
})
export class PlayerCityPoliticalNotesComponent implements OnInit, OnChanges, OnDestroy {
  @Input() campaignId!: string;
  @Input() cityInstanceId!: string;
  ...
  notes          = signal<CityPoliticalNotes>(EMPTY_NOTES());
  saving         = signal(false);
  openDropdownId = signal<string | null>(null);
  cityCastNotes  = signal<CampaignCastPlayerNotes[]>([]);
  ...
  selectedCastNotes = computed(() => {
    ...
    if (!cast) return null;
    return this.cityCastNotes().find(n => n.castInstanceId === cast.instanceId) ?? null;
  });
  ...
  availableCasts = computed(() => {
    const assigned = new Set(this.notes().npcRoles.map(n => n.castInstanceId));
    return this.allCasts().filter(c => !assigned.has(c.instanceId));
  });

  webNodes = computed(() =>
    this.notes().factions.map((f, i) => ({
      faction: f,
  ...
  webEdges = computed(() => {
    ...
    return this.notes().relationships.map(r => {
      const a = nodes.find(n => n.faction.id === r.factionAId);
      const b = nodes.find(n => n.faction.id === r.factionBId);
      if (!a || !b) return null;
  ...
  compassNodes = computed((): CompassNode[] => {
    const notesMap  = new Map(this.cityCastNotes().map(n => [n.castInstanceId, n]));
    ...
    return this.allCasts().map((cast, i) => {
      const note         = notesMap.get(cast.instanceId);
  ...
  compassEdges = computed((): CompassEdge[] => {
    const notes = this.cityCastNotes();
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['cityInstanceId'] && !changes['cityInstanceId'].firstChange) {
      this.cityCastNotes.set([]);
      this.load();
    }
  }
  ...
  private load() {
    this.http.get<CityPoliticalNotes>(
      `${environment.apiUrl}/api/campaigns/${this.campaignId}/city-political-notes/${this.cityInstanceId}`
    ).subscribe(n => {
      this.notes.set(n);
  ...
  private save() {
    ...
    const n = this.notes();
    this.http.put<CityPoliticalNotes>(
      `${environment.apiUrl}/api/campaigns/${this.campaignId}/city-political-notes/${this.cityInstanceId}`,
      {
        generalNotes: this.generalText,
        factions: n.factions.map(f => ({
          ...f,
  ...
  setFactionType(factionId: string, type: string) {
    this.notes.update(n => ({
      ...n, factions: n.factions.map(f => f.id === factionId ? { ...f, type } : f),
    }));
    this.closeDropdown();
    this.scheduleSave();
  }
  ...
  setFactionHidden(factionId: string, isHidden: boolean) {
    this.notes.update(n => ({
      ...n, factions: n.factions.map(f => f.id === factionId ? { ...f, isHidden } : f),
    }));
    this.scheduleSave();
  }
  ...
  removeFaction(id: string) {
    ...
    this.notes.update(n => ({
      ...n,
      factions:      n.factions.filter(f => f.id !== id),
      relationships: n.relationships.filter(r => r.factionAId !== id && r.factionBId !== id),
      npcRoles:      n.npcRoles.filter(nr => nr.factionId !== id),
    }));
    this.save();
  }
  ...
  setRelFactionA(relId: string, factionId: string) {
    this.notes.update(n => ({
      ...n, relationships: n.relationships.map(r => r.id === relId ? { ...r, factionAId: factionId } : r),
    }));
    this.closeDropdown();
    this.scheduleSave();
  }
  ...
  setRelFactionB(relId: string, factionId: string) {
    this.notes.update(n => ({
      ...n, relationships: n.relationships.map(r => r.id === relId ? { ...r, factionBId: factionId } : r),
    }));
    this.closeDropdown();
    this.scheduleSave();
  }
  ...
  setRelType(relId: string, type: string) {
    this.notes.update(n => ({
      ...n, relationships: n.relationships.map(r => r.id === relId ? { ...r, relationshipType: type } : r),
    }));
    this.closeDropdown();
    this.scheduleSave();
  }
  ...
  announceDistrict(faction: CityFaction) {
  ...
  private loadCityCastNotes() {
    ...
    ).subscribe(notes => this.cityCastNotes.set(notes));
  }
}
