...
export class CastLibraryComponent implements OnInit {
  ...
  ngOnInit() {
    console.log('CastLibrary: Initializing SignalR connection...');
    const token = this.auth.getToken();
    if (token && token.length > 0) {
      this.hub.connect(token).catch((err: unknown) => console.error('CastLibrary: SignalR connection failed:', err));
    } else {
      console.log('CastLibrary: No token found, skipping SignalR connection');
    }
